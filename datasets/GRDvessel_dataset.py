...
class VesselDetectGRDDataset(Dataset):
    def __init__(
        self,
        mode: Literal["train", "valid", "test"],
        use_sen1: bool,
        dataset_dir: str = "/projects/0/prjs1235/FMforSAR/data/GRD",
        csv_dir: str = "splits",
        sen1_dir: str = "/projects/0/prjs1235/FMforSAR/data/GRD/sampled_tiles_vvvh_overlap_224/sar",
        label_dir: str = "/projects/0/prjs1235/FMforSAR/data/GRD/sampled_tiles_vvvh_overlap_224/masks",
    ):
        ...
        df = pd.read_csv(
            os.path.join(dataset_dir, csv_dir, f"vessel_{mode}_data.csv"), header=None
        )

        # Format of sample names: {country}_{chip_id}
        self.sample_names = [fn.replace("sar_", "") for fn in df[0]]
    ...
    def get_labels(self, idx):
        ...
        mask_path = os.path.join(self.label_dir, f"mask_{self.sample_names[idx]}")
    ...
    def get_sen1_img(self, idx):
        sen1_path = os.path.join(self.sen1_dir, f"sar_{self.sample_names[idx]}")
